  ...
  return (
    <div className='flex lg:flex-col gap-2 p-2 bg-primary/10 rounded-md lg:order-first overflow-auto '>
        { options.map(({ title, url, icon }) => (
            <Link href={`/categories/${url}`} key={title} className='lg:flex items-center gap-2 lg:pr-2 rounded-md  h-[100px] min-w-[150px] lg:h-auto lg:w-auto overflow-hidden shadow-custom bg-white border border-primary/10 transition ease-in-out duration-300 hover:border-primary/80'> 
